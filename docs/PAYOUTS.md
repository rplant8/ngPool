...
* Submit a transaction to a node via `ngin_sendTransaction`
...
If there was a debit operation performed which is not followed by actual money transfer (after `ngin_sendTransaction` returned an error), you will likely see:
